...
router.post("/", async (req, res) => {
  const { nome, cargo, idevent } = req.body;

  try {
    const queryResult = await connection.query(
      "INSERT INTO volunteers (nome, cargo, idevent) VALUES ($1, $2, $3) RETURNING *",
      [nome, cargo, idevent]
    );
...
router.get("/:eventid", async (req, res) => {
  ...
  try {
    const queryResult = await connection.query(
      "SELECT * FROM volunteers WHERE idevent = $1",
      [eventid]
    );
    res.json(queryResult.rows);
  } catch (e) {
...
router.delete("/delete/:idevent", async (req, res) => {
  ...
  try {
    await connection.query("DELETE FROM volunteers WHERE idevent = $1", [
      idevent,
    ]);
    res.status(204).send();
  } catch (e) {
...
router.get("/id/maxid", async (req, res) => {
  try {
    const queryResult = await connection.query(
      "SELECT MAX(id) AS maxid FROM volunteers"
    );
