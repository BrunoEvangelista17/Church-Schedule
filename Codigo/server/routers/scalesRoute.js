...
router.get("/membro/:id", async (req, res) => {
  ...
  try {
    const queryResult = await connection.query(
      "SELECT * FROM scale WHERE idmembro = $1",
      [id]
    );
    res.json(queryResult.rows);
  } catch (e) {
...
router.get("/diaconia/:id", async (req, res) => {
  ...
  try {
    const queryResult = await connection.query(
      "SELECT * FROM scale WHERE idmembro = $1 AND idcargos BETWEEN 12 AND 16",
      [id]
    );
    res.json(queryResult.rows);
  } catch (e) {
...
router.get("/louvor/:id", async (req, res) => {
  ...
  try {
    const queryResult = await connection.query(
      "SELECT * FROM scale WHERE idmembro = $1 AND (idcargos BETWEEN 1 AND 6 OR idcargos = 17)",
      [id]
    );
    res.json(queryResult.rows);
  } catch (e) {
...
router.get("/midia/:id", async (req, res) => {
  ...
  try {
    const queryResult = await connection.query(
      "SELECT * FROM scale WHERE idmembro = $1 AND idcargos BETWEEN 7 AND 11",
      [id]
    );
    res.json(queryResult.rows);
  } catch (e) {
...
router.get("/diaconia/:idmembro/evento/:idevento", async (req, res) => {
  ...
  try {
    const queryResult = await connection.query(
      "SELECT * FROM scale WHERE idmembro = $1 AND (idcargos BETWEEN 12 AND 16) AND ideventos = $2",
      [idmembro, idevento]
    );
    res.json(queryResult.rows);
  } catch (e) {
...
router.get("/louvor/:idmembro/evento/:idevento", async (req, res) => {
  ...
  try {
    const queryResult = await connection.query(
      "SELECT * FROM scale WHERE idmembro = $1 AND (idcargos BETWEEN 1 AND 6 OR idcargos = 17) AND ideventos = $2",
      [idmembro, idevento]
    );
    res.json(queryResult.rows);
  } catch (e) {
...
router.get("/midia/:idmembro/evento/:idevento", async (req, res) => {
  ...
  try {
    const queryResult = await connection.query(
      "SELECT * FROM scale WHERE idmembro = $1 AND (idcargos BETWEEN 7 AND 11) AND ideventos = $2",
      [idmembro, idevento]
    );
    res.json(queryResult.rows);
  } catch (e) {
...
router.post("/", async (req, res) => {
  const { idCargos, idEventos, idMembro } = req.body;

  try {
    const queryResult = await connection.query(
      "INSERT INTO scale (idCargos, idEventos, idMembro) VALUES ($1, $2, $3) RETURNING *",
      [idCargos, idEventos, idMembro]
    );
...
router.get("/:id", async (req, res) => {
  ...
  try {
    const queryResult = await connection.query(
      "SELECT * FROM scale WHERE ideventos = $1",
      [id]
    );
    res.json(queryResult.rows);
  } catch (e) {
...
router.post("/edit/:id", async (req, res) => {
  const { id, idcargos, ideventos, idmembro } = req.body;
  // const { id } = req.params;
...
router.get("/min/:mindate/max/:maxdate", async (req, res) => {
  ...
  try {
    const queryResult = await connection.query(
      "SELECT s.*, m.name, m.diaconia, m.midia, m.louvor FROM scale s JOIN events e ON s.ideventos = e.id JOIN members m ON s.idmembro = m.id WHERE e.dateevent BETWEEN $1 AND $2",
      [mindate, maxdate]
    );
    res.json(queryResult.rows);
  } catch (e) {
